...
-- TODO: update the Target.HVM module to use the new Core.Term module.
-- note that, in the previous implementation, effort was made to convert
-- sigma-based datatypes into proper datatypes (that HVM uses). now,
-- since Bend features native datatypes, this isn't needed, making
-- the translation more straightforward in that regard.
...
compile :: Book -> String
compile book@(Book funs adts names) =
  -- TODO: Error handling
  if M.notMember "main" funs then
    error "No main function found"
  else
    let ts      = map (compileADT book) (M.toList adts)
        fs      = map (compileFun book) (M.toList funs)
        main    = "@main = " ++ showHVM 1 (termToHVM book (Ref "main" 1)) ++ "\n\n"
    in prelude ++ main ++ unlines ts ++ unlines fs
...
-- Compile a Bend ADT to an HVM data declaration
compileADT :: Book -> (Name, DataType) -> String
compileADT book (nam, DataType adtName adtType ctrs) =
...
-- Compile a Bend function to an HVM definition
compileFun :: Book -> (Name, Function) -> String
compileFun book (nam, Function funName funType funBody) =
  "!@" ++ (defNam nam) ++ "(" ++ unwords argsStr ++ ") =\n  " ++ showHVM 1 bodHVM ++ "\n"
  where
    (eras,args,body) = collectLamArgs funBody funType [] []
    argsStr          = map (\(k,ty) -> if isStri k ty then "!&"++k else "&"++k) args
    bodHVM           = foldr (\era x -> rewriteHVM (HVM.Var era) HVM.Era x) (termToHVM book body) eras
    isStri k ty      = alwaysMat k body || isNum ty
    isNum ty         = case ty of { Num _ -> True; _ -> False }
    alwaysMat x bod  = case cut bod of
      -- Matches this var
      App (BitM _ _) (cut -> Var y _)  | x == y -> True
      App (NatM _ _) (cut -> Var y _)  | x == y -> True
...
termToHVM book term = go term where
  ...
  go (Fix n f)     = HVM.Ref "fix" 0 [HVM.Lam (bindNam n) (termToHVM book (f (Var n 0)))]
  go (Let k t v f) = HVM.Let HVM.LAZY (bindNam k) (termToHVM book v) (termToHVM book (f (Var k 0)))
  go (Use k v f)   = termToHVM book (f v)
  go Set           = HVM.Era
  go (Chk v t)     = termToHVM book v
  go Emp           = HVM.Era
  go EmpM          = HVM.Lam "x$" HVM.Era
  ...
  go (Con h t)     = HVM.Ctr "#Cons" [termToHVM book h, termToHVM book t]
  go (LstM n c)    = HVM.Lam "x$" $ HVM.Mat (HVM.MAT 0) (HVM.Var "x$") [] [("#Nil", [], termToHVM book n), ("#Cons", ["x$h", "x$t"], HVM.App (HVM.App (termToHVM book c) (HVM.Var "x$h")) (HVM.Var "x$t"))]
  go (Log s x)     = termToHVM book x
  go (Num _)       = HVM.Era
  go (Val v)       = valToHVM book v
  go (Op2 o a b)   = op2ToHVM book o a b
  go (Op1 o a)     = op1ToHVM book o a
  go (Sig _ _)     = HVM.Era
  go (Tup x y)     = HVM.Sup 0 (termToHVM book x) (termToHVM book y)
  go (SigM f)   =
    case f of
      (Lam l _ (subst l -> (Lam r _ (subst r -> f)))) ->
        HVM.Lam "x$" (HVM.Dup 0 l r (HVM.Var "x$") (termToHVM book f))
  ...
  go Rfl          = HVM.Era
  go (EqlM _)     = HVM.Era
  go (ADT n args) = HVM.Era  -- ADT types erase at runtime
  go (Ctr n args) = HVM.Ctr ('#':defNam n) (map (termToHVM book) args)
  go (ADTM adtName cases defCase) = adtmToHVM book adtName cases defCase
  go (Met n t ts) = HVM.Era
  go Era          = HVM.Era
  go (Sup l a b)  =
    HVM.Let HVM.LAZY "sup0$" (termToHVM book a) $
    HVM.Let HVM.LAZY "sup1$" (termToHVM book b) $
...
-- Convert ADTM to HVM
adtmToHVM :: Book -> Name -> [(Name, Term)] -> Maybe Term -> HVM.Core
adtmToHVM book adtName cases defCase = 
  HVM.Lam "x$" $ HVM.Mat (HVM.MAT 0) (HVM.Var "x$") [] hvmCases
  where
    hvmCases = map caseToHVM cases ++ defCaseToHVM defCase
    caseToHVM (ctrName, body) = 
      let fields = getCtrFields ctrName
      in ('#':defNam ctrName, fields, termToHVM book body)
    defCaseToHVM Nothing = []
...
refAppToHVM book term =
  case collectApps term [] of
    (Ref k i, args) ->
      let funDef = case getFun book k of
            Just f -> f
            Nothing -> error ("unknown ref in hvm backend: " ++ show k)
          (era,arg,_) = collectLamArgs (funBody funDef) (funType funDef) [] []
          argsHVM     = map (termToHVM book) (drop (length era) args)
          ari         = length arg
          len         = length argsHVM
      in return $ wrapRef (defNam k) argsHVM len ari
...
patToHVM book [x] m c@(([p], f) : _) =
  case p of
    (Var k i) -> HVM.Let HVM.LAZY (bindNam k) (termToHVM book x) (termToHVM book f)
    (Emp)     -> HVM.Era
    (One)     -> termToHVM book f
    (Bt0)     -> bitMat
    (Bt1)     -> bitMat
    ...
    (Ctr _ _) -> adtMat
    (Tup (Var a _) (Var b _)) ->
      HVM.Dup 0 a b (termToHVM book x) (termToHVM book f)
    (Rfl)     -> termToHVM book (snd (head c))
    (Sup l (Var a _) (Var b _)) ->
      HVM.Ref "DUP" 0 [termToHVM book l, termToHVM book x, (HVM.Lam (bindNam a) (HVM.Lam (bindNam b) (termToHVM book f)))]
    _ -> HVM.Era
  where
    -- A match that is contained in a single Pat term (bool, nat, list, ADT)
    simpleMat = HVM.Mat (HVM.MAT 0) (termToHVM book x) hvmMv sortedCs
    hvmMv     = map (\(k,v) -> (bindNam k, termToHVM book v)) m
    -- Bend can output first True (1/_) and then False (0), so need to sort.
    sortedCs  = sortBy (\(a,_,_) (b,_,_) -> compare a b) hvmCs
    ...
    bitMat = case c of
      ...
      _ -> HVM.Era
      where tm f t = HVM.Mat HVM.SWI (termToHVM book x) hvmMv [("0", [], f), ("1+bp$", [], t)]
    adtMat = HVM.Mat (HVM.MAT 0) (termToHVM book x) hvmMv hvmAdtCs
      where
        hvmAdtCs = map (\(p,x) -> case head p of
            (Ctr ctrName args) -> 
              let fields = extractCtrVars args
              in ('#':defNam ctrName, map bindNam fields, termToHVM book x)
            (Var k _) -> (bindNam k, [], termToHVM book x)
            _ -> ("_", [], HVM.Era)
          ) goodCs
        extractCtrVars [] = []
        extractCtrVars ((Var k _):xs) = k : extractCtrVars xs
        extractCtrVars (_:xs) = extractCtrVars xs
patToHVM book x m c = HVM.Era

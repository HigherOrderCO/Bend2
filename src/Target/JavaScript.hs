...
-- TODO: update the file above based on the new Term type
...
-- data CT
  ...
  -- | CBt1
  -- | CTup CT CT
  -- | CCtr String [CT]  -- ADT constructor
  -- -- Pattern matching lambdas
  -- | CSwi CT CT
  -- | CBif CT CT
  -- | CMat CT CT
  -- | CAdtM String [(String, CT)] (Maybe CT)  -- ADT match
  -- | CGet CT
  -- | CUse CT
  -- | CEql CT
  -- -- Numeric
...
-- termToCT book term dep = case term of
  ...
  -- Frk l a b    -> CFrk (termToCT book l dep) (termToCT book a dep) (termToCT book b dep)
  -- Rwt _ _      -> CEra  -- Erases at runtime
  -- ADT _ _      -> CEra  -- Type references erase
  -- Ctr n args   -> CCtr n (map (\a -> termToCT book a dep) args)
  -- ADTM t cases def -> 
    -- let convCases = map (\(k,v) -> (k, termToCT book v dep)) cases
        -- convDef = fmap (\d -> termToCT book d dep) def
    -- in CLam "x$" (\x -> CApp (CAdtM t convCases convDef) x)
  -- Pat _ _ _  -> error "unreachable"
...
-- compileConstructor var (CTup a b) dep = do
  ...
  -- setStmt <- set var $ "[" ++ aName ++ ", " ++ bName ++ "]"
  -- return $ concat [aStmt, bStmt, setStmt]
-- compileConstructor var (CCtr name args) dep = do
  -- argNamesStmts <- forM (zip [0..] args) $ \(i, arg) -> do
    -- argName <- fresh
    -- argStmt <- ctToJS' False argName arg dep
    -- return (argName, argStmt, "$" ++ show i ++ ":" ++ argName)
  -- let fields = intercalate ", " (map (\(_,_,f) -> f) argNamesStmts)
  -- setStmt <- set var $ "{$:\"" ++ name ++ "\", " ++ fields ++ "}"
  -- return $ concat (map (\(_,s,_) -> s) argNamesStmts ++ [setStmt])
-- compileConstructor _ _ _ = error "compileConstructor: not a constructor"
...
-- -- Compile ADT elimination
-- compileAdtM :: CTBook -> String -> [String] -> Bool -> String -> String -> [(String, CT)] -> Maybe CT -> CT -> Int -> JSM String
-- compileAdtM book fnName fnArgs isTail var adtName cases defCase val dep = do
  -- valName <- fresh
  -- valStmt <- ctToJS' False valName val dep
  
  -- -- Generate switch cases for each constructor
  -- casesCode <- forM cases $ \(ctrName, ctrBody) -> do
    -- -- Get argument names
    -- let getFieldAccess i = valName ++ ".$" ++ show i
    -- -- Count lambda depth to know how many fields to extract
    -- let countLams (CLam _ f) = 1 + countLams (f (CVar "" 0))
        -- countLams _ = 0
    -- let numFields = countLams ctrBody
    
    -- -- Apply the case body to the field values
    -- let applyFields :: CT -> Int -> Int -> JSM String
        -- applyFields body 0 _ = ctToJS book fnName fnArgs isTail var body dep
        -- applyFields (CLam param f) n fieldIdx = do
          -- let fieldVar = getFieldAccess fieldIdx
          -- let paramName = param
          -- restStmt <- applyFields (f (CVar paramName (dep + fieldIdx))) (n - 1) (fieldIdx + 1)
          -- return $ "var " ++ paramName ++ " = " ++ fieldVar ++ "; " ++ restStmt
        -- applyFields body n fieldIdx = do
          -- -- If not enough lambdas, apply what we have
          -- bodyName <- fresh
          -- bodyStmt <- ctToJS' False bodyName body dep
          -- let applyField idx acc = CApp acc (CVar (getFieldAccess idx) dep)
          -- let applied = foldl applyField (CVar bodyName dep) [0..n-1]
          -- appStmt <- ctToJS book fnName fnArgs isTail var applied dep
          -- return $ bodyStmt ++ appStmt
    
    -- caseStmt <- applyFields ctrBody numFields 0
    -- return $ "case \"" ++ ctrName ++ "\": { " ++ caseStmt ++ " break; }"
  
  -- -- Handle default case
  -- defaultCode <- case defCase of
    -- Nothing -> return ""
    -- Just def -> do
      -- -- Default case gets the entire value
      -- defStmt <- ctToJS book fnName fnArgs isTail var (CApp def (CVar valName dep)) dep
      -- return $ "default: { " ++ defStmt ++ " }"
  
  -- let switch = "switch (" ++ valName ++ ".$) { "
  -- let joined = intercalate " " casesCode ++ (if null defaultCode then "" else " " ++ defaultCode)
  -- return $ valStmt ++ switch ++ joined ++ " }"
...
-- compileApp book fnName fnArgs isTail var app dep = do
  ...
  -- else case appFun of
    ...
    -- CMat n c | length appArgs == 1 -> 
      -- compileMat book fnName fnArgs isTail var n c (head appArgs) dep
    -- CAdtM adtName cases defCase | length appArgs == 1 -> 
      -- compileAdtM book fnName fnArgs isTail var adtName cases defCase (head appArgs) dep
    -- CGet f | length appArgs == 1 -> 
      -- compileGet book fnName fnArgs isTail var f (head appArgs) dep
    -- CUse f | length appArgs == 1 -> 
      -- compileUse book fnName fnArgs isTail var f (head appArgs) dep
...
-- ctToJS book fnName fnArgs isTail var term dep = case term of
  ...
  -- CCon _ _    -> compileConstructor var term dep
  -- CTup _ _    -> compileConstructor var term dep
  -- CCtr _ _    -> compileConstructor var term dep
  -- CVal v      -> compileNumeric var v
  -- COp2 op a b -> compileBinOp var op a b dep
  -- COp1 op a   -> compileUnOp var op a dep
  -- CLam _ _    -> compileLambda var term dep
...
-- -- Compile book to JavaScript
-- compile :: Book -> String
-- compile (Book funs adts names) =
  -- let ctDefs = map (\(name, fn) -> (name, termToCT (Book funs adts names) (funBody fn) 0)) (M.toList funs)
      -- ctBook = M.fromList ctDefs
      -- jsFns = concatMap (generateJS ctBook) ctDefs
  -- in prelude ++ jsFns


-- src/Target/JavaScript.hs:943:31: error: [GHC-83865]
    -- • Couldn't match type ‘CT’ with ‘Int’
      -- Expected: Int -> CT -> Int
        -- Actual: Int -> CT -> CT
    -- • In the first argument of ‘foldl’, namely ‘applyField’
      -- In the expression:
        -- foldl applyField (CVar bodyName dep) [0 .. n - 1]
      -- In an equation for ‘applied’:
          -- applied = foldl applyField (CVar bodyName dep) [0 .. n - 1]
    -- |
-- 943 |           let applied = foldl applyField (CVar bodyName dep) [0..n-1]
    -- |                               ^^^^^^^^^^

-- src/Target/JavaScript.hs:943:43: error: [GHC-83865]
    -- • Couldn't match expected type ‘Int’ with actual type ‘CT’
    -- • In the second argument of ‘foldl’, namely ‘(CVar bodyName dep)’
      -- In the expression:
        -- foldl applyField (CVar bodyName dep) [0 .. n - 1]
      -- In an equation for ‘applied’:
          -- applied = foldl applyField (CVar bodyName dep) [0 .. n - 1]
...
-- src/Target/JavaScript.hs:943:66: error: [GHC-83865]
    -- • Couldn't match expected type ‘CT’ with actual type ‘Int’
    -- • In the first argument of ‘(-)’, namely ‘n’
      -- In the expression: n - 1
      -- In the third argument of ‘foldl’, namely ‘[0 .. n - 1]’
    -- |
-- 943 |           let applied = foldl applyField (CVar bodyName dep) [0..n-1]
    -- |                                                                  ^

-- src/Target/JavaScript.hs:944:59: error: [GHC-83865]
    -- • Couldn't match expected type ‘CT’ with actual type ‘Int’
    -- • In the sixth argument of ‘ctToJS’, namely ‘applied’
      ...
      -- In the expression:
        -- do bodyName <- fresh
           -- bodyStmt <- ctToJS' False bodyName body dep
           -- let applyField idx acc = CApp acc (CVar (getFieldAccess idx) dep)
           -- let applied = foldl applyField (CVar bodyName dep) ...
           -- ....
    -- |
-- 944 |           appStmt <- ctToJS book fnName fnArgs isTail var applied dep
    -- |                                                           ^^^^^^^
...
compileConstructor var (CTup a b) dep = do
  ...
  return $ concat [aStmt, bStmt, setStmt]
compileConstructor var (CCtr name args) dep = do
  argNamesStmts <- forM (zip [(0::Int)..] args) $ \(i, arg) -> do
    argName <- fresh
    argStmt <- ctToJS' False argName arg dep
    return (argName, argStmt, "$" ++ show i ++ ":" ++ argName)
  let fields = intercalate ", " (map (\(_,_,f) -> f) argNamesStmts)
  ...
  casesCode <- forM cases $ \(ctrName, ctrBody) -> do
    ...
    let applyFields :: CT -> Int -> Int -> JSM String
        ...
        applyFields body n fieldIdx = do
          ...
          bodyName <- fresh
          bodyStmt <- ctToJS' False bodyName body dep
          let applied = foldl (\acc idx -> CApp acc (CVar (getFieldAccess idx) dep)) (CVar bodyName dep) [0..n-1]
          appStmt <- ctToJS book fnName fnArgs isTail var applied dep
          return $ bodyStmt ++ appStmt
...
-- Compile normal application
compileNormalApp :: String -> CT -> [CT] -> Int -> JSM String
compileNormalApp var fun [] dep = ctToJS' False var fun dep
compileNormalApp var fun (arg:args) dep = do
  funName <- fresh
  funStmt <- ctToJS' False funName fun dep
  argName <- fresh

...
-- parseDefinition = do
  -- result <- choice [ parseType , parseDef , parseTry , parseAssert ]
  -- return result
...
-- parseBook = do
  ...
  -- -- Then parse definitions
  -- defs <- many parseDefinition
  -- let functions = M.fromList [(funName f, f) | Left f <- defs]
  -- let datatypes = M.fromList [(adtName d, d) | Right d <- defs]
  -- let names = [funName f | Left f <- defs] ++ [adtName d | Right d <- defs]
  -- return $ Book functions datatypes names

-- -- | Syntax: type Name<T, U> -> Type: case Ctr{}: field1: T1 case Ctr2{}: field2: T2
...
-- parseType = label "datatype declaration" $ do
  ...
  -- let adtType = foldr (\(n,ty) acc -> All ty (Lam n (Just ty) (\_ -> acc))) retTy params
  -- -- Build the constructors
  -- let buildCtr (ctrName, fields) = 
        -- let mkCtrType p = foldr (\(fn,ft) acc -> All ft (Lam fn (Just ft) (\_ -> acc))) 
                               -- (App p (Ctr ctrName (map (Var . fst) fields))) 
                               -- fields
        -- in (ctrName, mkCtrType)
  -- let ctrs = map buildCtr cases
  -- -- Also add a convenience function for the ADT
  -- let funType = adtType
  -- let funBody = foldr (\(n,ty) acc -> Lam n (Just ty) (\v -> acc)) (ADT tName (map (Var . fst) params)) params
  -- let function = Function tName funType funBody
  -- return (Right $ DataType tName adtType ctrs)
  -- where
    -- parseArg :: Bool -> Parser (Name, Term)
    -- parseArg expr = do
      -- k <- name
...
-- parseTypeCase = label "datatype constructor" $ do
  ...
  -- where
    ...
    -- parseField :: Parser (Name, Term)
    -- parseField = do
      -- -- Stop if next token is 'case' (start of next constructor) or 'def'/'type'
      -- notFollowedBy (symbol "case")
      -- n <- try $ do
        -- n <- name
        -- _ <- symbol ":"
...
-- src/Core/Parse/Book.hs:369:58: error: [GHC-83865]
    -- • Couldn't match type ‘Int -> Term’ with ‘Term’
      -- Expected: Name -> Term
        -- Actual: Name -> Int -> Term
    -- • Probable cause: ‘Var’ is applied to too few arguments
      -- In the first argument of ‘(.)’, namely ‘Var’
      -- In the first argument of ‘map’, namely ‘(Var . fst)’
      -- In the second argument of ‘Ctr’, namely ‘(map (Var . fst) fields)’
    -- |
-- 369 |                                (App p (Ctr ctrName (map (Var . fst) fields)))
    -- |                                                          ^^^

-- src/Core/Parse/Book.hs:375:85: error: [GHC-83865]
    -- • Couldn't match type ‘Int -> Term’ with ‘Term’
      -- Expected: Name -> Term
        -- Actual: Name -> Int -> Term
    -- • Probable cause: ‘Var’ is applied to too few arguments
      -- In the first argument of ‘(.)’, namely ‘Var’
      -- In the first argument of ‘map’, namely ‘(Var . fst)’
      -- In the second argument of ‘ADT’, namely ‘(map (Var . fst) params)’
    -- |
-- 375 |   let funBody = foldr (\(n,ty) acc -> Lam n (Just ty) (\v -> acc)) (ADT tName (map (Var . fst) params)) params
    -- |

-- also the new code isn't simple nor idiomatic. improve it. make it correct.
...
-- parseDefFunction f = label "function definition" $ do
  ...
  -- where
    -- nestTypeBod (argName, argType) (currType, currBod) = 
      -- (All argType (Lam argName (Just argType) (\v -> currType)), 
       -- Lam argName (Just argType) (\v -> currBod))
...
-- parseType = label "datatype declaration" $ do
  ...
  -- -- Build constructors with the format: λp. ∀ field:Type . p(Ctr{fields})
  -- let buildCtr (ctrName, fields) = 
        -- let ctrType p = 
              -- let vars = [Var ("_f" ++ show i) 0 | (i, _) <- zip [0..] fields]
                  -- ctrApp = App p (Ctr ctrName vars)
              -- in foldr (\((fn,ft), v) acc -> All ft (Lam fn (Just ft) (\_ -> acc))) 
                      -- ctrApp 
                      -- (zip fields vars)
        -- in (ctrName, ctrType)
  ...
  -- let funType = adtType
  -- let paramVars = [Var n 0 | (n, _) <- params]
...
-- parseTryFunction :: Name -> Parser Function
-- parseTryFunction nam = label "try definition" $ do
  -- tyParams <- option [] $ angles $ sepEndBy name (symbol ",")
  -- regularArgs <- parens $ sepEndBy parseArg (symbol ",")
  -- let args = [(tp, Set) | tp <- tyParams] ++ regularArgs
...
-- src/Core/Parse/Book.hs:696:5: error: [GHC-83865]
    -- • Couldn't match expected type ‘Function’
                  -- with actual type ‘(Term, Type)’
    -- • In the pattern: (x, t)
      ...
      -- In the second argument of ‘($)’, namely
        -- ‘do _ <- symbol "try"
            ...
            -- return (f, x, t)’
    -- |
-- 696 |     (x, t) <- choice [parseTryFunction f, parseTrySimple f]
    -- |     ^^^^^^

-- src/Core/Parse/Book.hs:696:43: error: [GHC-83865]
    -- • Couldn't match type ‘(Term, Type)’ with ‘Function’
      -- Expected: ParsecT
                  -- Void String (Control.Monad.State.Strict.State ParserState) Function
        -- Actual: Parser (Term, Type)
    -- • In the expression: parseTrySimple f
      -- In the first argument of ‘choice’, namely
        -- ‘[parseTryFunction f, parseTrySimple f]’
      -- In a stmt of a 'do' block:
        -- (x, t) <- choice [parseTryFunction f, parseTrySimple f]
    -- |
-- 696 |     (x, t) <- choice [parseTryFunction f, parseTrySimple f]
...
-- src/Core/Parse/Book.hs:716:10: error: [GHC-83865]
    -- • Couldn't match expected type ‘Function’
                  -- with actual type ‘(Term, Term)’
    -- • In the first argument of ‘return’, namely
        -- ‘(Met nam typ ctx, typ)’
      -- In a stmt of a 'do' block: return (Met nam typ ctx, typ)
      -- In the second argument of ‘($)’, namely
        -- ‘do tyParams <- option [] $ angles $ sepEndBy name (symbol ",")
            -- regularArgs <- parens $ sepEndBy parseArg (symbol ",")
            -- let args = ... ++ regularArgs
            -- _ <- symbol "->"
            -- ....’
    -- |
-- 716 |   return (Met nam typ ctx, typ)
    -- |          ^^^^^^^^^^^^^^^^^^^^^^

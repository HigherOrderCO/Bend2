...
parseTermIni = choice
  ...
  , parseNil
  , parseRfl
  , parseADT      -- NEW: type T<p0,p1,...>
  , parseCtr      -- NEW: K{v0,v1,...}
  , parseTupApp
  , parseView
  , parseVar
  ]

-- | Parse a "tight" postfix: f(...) | f[] | f<...> ...
parseTermArg :: Term -> Parser Term
parseTermArg t = do
  st <- get
  guard (tight st)
...
-- ADT Type Parsers (NEW)
-- ----------------------

-- | Syntax: type T<p0,p1,...>
parseADT :: Parser Term
parseADT = label "ADT type" $ try $ do
  _ <- keyword "type"
  n <- name
  _ <- symbol "<"
  args <- sepEndBy parseTerm (symbol ",")
  ...
  case mb_close of
    ...
    Nothing -> do
      term <- choice
        ...
        , parseADTMForm   -- NEW
        , parseSupMForm
        , parseEqlMForm   -- NEW: equality match
        ]
      _ <- symbol "}"
      return term
  where
    ...
    -- λT{ K{}: K_case ; ... ; {}: def_case } (NEW)
    parseADTMForm = do
      -- Check if this starts with a name followed by {
      adtName <- try $ do
        n <- lookAhead name
        skip
        _ <- lookAhead (symbol "{")
        name
      _ <- symbol "{"
      -- Parse constructor cases
      ctrCases <- many $ try parseCtrCase
      -- Parse optional default case
      defCase <- optional $ try parseDefCase
      return (ADTM adtName ctrCases defCase)
      where
        parseCtrCase = do
          ctrName <- name
          args <- option [] $ do
            _ <- symbol "{"
            args <- sepEndBy name (symbol ",")
            _ <- symbol "}"
            return args
          _ <- symbol ":"
          body <- parseTerm
          _ <- parseSemi
          -- Desugar K{x0,x1,...}: body to K{}: λx0. λx1. ... body
          let desugaredBody = foldr (\arg acc -> Lam arg Nothing (\_ -> acc)) body args
          return (ctrName, desugaredBody)
        ...
        parseDefCase = do
          ...
          body <- parseTerm
          _ <- parseSemi
          -- Desugar x: body to {}: λx. body
          case arg of
            Nothing -> return body
            Just x  -> return (Lam x Nothing (\_ -> body))
            ...
            case mb_close of
              ...
              Nothing -> do
                term <- choice
                  ...
                  , parseLstMCases scrut
                  , parseSigMCases scrut
                  , parseADTMCases scrut  -- NEW
                  , parseSupMCases scrut
                  , parseEqlMCases scrut  -- NEW
                  ]
                _ <- symbol "}"
                return term
          Nothing -> fail "Expected match expression after ~"
...
-- | Syntax: T{ K{}: K_case ; ... ; {}: def_case } (NEW)
parseADTMCases :: Term -> Parser Term
parseADTMCases scrut = do
  -- Parse ADT name
  adtName <- try $ do
    n <- lookAhead name
    skip
  ...
  defCase <- optional $ try parseDefCase
  _ <- parseSemi
  return (App (ADTM adtName ctrCases defCase) scrut)
  where
    parseCtrCase = do
      ctrName <- name
      args <- option [] $ do

...
type CountMap = (M.Map Name Int,M.Map Name Int)
...
variationsMap book@(Book defs nams) =
  ...
  where
    ...
    initialCounts = (M.fromList $ map (\nam -> (shortName nam, 1)) nams, M.empty)
    ...
    go d count term = case term of
      ...
      -- Enum
      Enu s        -> mergeCounts count (M.empty,(M.fromList $ map (\s -> (shortName s,1)) s))
      Sym s        -> emptyCount

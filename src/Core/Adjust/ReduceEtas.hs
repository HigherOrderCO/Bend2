  ...
  Lam k mty f ->
    case isEtaLong d k (f (Var k d)) of
      ...
      ENUM syms compl -> EnuM cases def where
        cases = map (\sym -> (sym, Use k (Sym sym) (\x -> reduceEtas d (resolveMatches d k (ENUM syms compl) 0 sym [] (f (Var k d)))))) syms
        def = if compl
          then reduceEtas d (Lam (k++"$def") Nothing (\q ->
            Use k q (\x -> (resolveMatches (d+1) k (ENUM syms compl) (-1) "" [q] (f (Var k d))))))
          else One
        -- def = 
        --   let newDef = Lam (k++"$def") Nothing (\v -> (f v))
        --   in reduceEtas d newDef
        -- def = case cut def of
        --   Lam defK mtdef defb ->
        --     let newDef = Lam defK mtdef (\v -> bindVarByIndex d v (f (Var defK (d+1))))
        --     -- in reduceEtas d newDef
        --     in newDef
        --   _ -> def
...
resolveMatches d n l clause sym args t = case t of
  ...
  Fix name f -> Fix name (\x -> resolveMatches (d+1) n l clause sym args (f x))
  Let name ty v f -> Let name (fmap (resolveMatches d n l clause sym args) ty) 
                       (resolveMatches d n l clause sym args v) 
                       (\x -> resolveMatches (d+1) n l clause sym args (f x))
  Use name v f -> Use name (resolveMatches d n l clause sym args v) 
                      (\x -> resolveMatches (d+1) n l clause sym args (f x))
  ...
  Enu syms -> Enu syms
  Sym s -> Sym s
  EnuM cs def -> EnuM (map (\(s,t') -> (s, resolveMatches d n l clause sym args t')) cs) 
                      (resolveMatches d n l clause sym args def)
  Num typ -> Num typ
  Val v -> Val v
    ...
    case cut x of
      (Var k i) -> if k == n
        then case (cut f, l) of
          ...
          (BitM fl tr,    _) -> BitM (resolveMatches d n l clause sym args fl) (resolveMatches d n l clause sym args tr)
          ...
          (NatM z s,    _) -> NatM (resolveMatches d n l clause sym args z) (resolveMatches d n l clause sym args s)
          ...
          (LstM nil cons, _) -> LstM (resolveMatches d n l clause sym args nil) (resolveMatches d n l clause sym args cons)
          ...
          (SigM pair, _) -> SigM (resolveMatches d n l clause sym args pair)         
          ...
          (SupM label branches, _) -> SupM (resolveMatches d n l clause sym args label) (resolveMatches d n l clause sym args branches)
          ...
          (EnuM cs def, ENUM syms _) ->
            if clause == -1 
              then case (cut def, args) of
                 (Lam _ _ body, [q]) -> trace ("probing: " ++ show (body (Var "Probe" 0))) $ resolveMatches (d+1) n l clause sym args (body q)
      ...
      _         -> App (resolveMatches d n l clause sym args f) (resolveMatches d n l clause sym args x)
  Eql ty a b -> Eql (resolveMatches d n l clause sym args ty) 
  ...
  Met name ty as -> Met name (resolveMatches d n l clause sym args ty) 
                          (map (resolveMatches d n l clause sym args) as)
  Era -> Era
  Sup l' a b -> Sup (resolveMatches d n l clause sym args l') 
                    (resolveMatches d n l clause sym args a) 
                    (resolveMatches d n l clause sym args b)
  ...
  Pat ss ms cs -> Pat (map (resolveMatches d n l clause sym args) ss)
                     ...
                     (map (\(ps,rhs) -> (map (resolveMatches d n l clause sym args) ps, 
                                        resolveMatches d n l clause sym args rhs)) cs)
...
isEtaLong d n t = case t of
  ...
  --       SupM l _ -> (SUPM l)
  --       EqlM _   -> EQLM
  --       EnuM cs _ -> (ENUM (map fst cs))  -- Extract symbol order
  --       _      -> isEtaLong d n f
  --     else isEtaLong d n f <+> isEtaLong d n x
  --   _         -> isEtaLong d n f <+> isEtaLong d n x
  ...
  Met _ ty args -> isEtaLong d n ty <+> foldr (<+>) NONE (map (isEtaLong d n) args)
  Era -> NONE

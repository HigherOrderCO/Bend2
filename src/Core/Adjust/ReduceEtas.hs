...
reduceEtas d span term = case term of
  Lam k mty f ->
      ...
      case isEtaLong d span nam (f (Var nam d)) of
        ...
        EQLM -> EqlM refl 
            where
            refl = reduceEtas d span $ bindVarByName nam Rfl (resolveMatches d span nam EQLM 0 "" [] (f (Var nam d)))
        UNIM -> UniM u 
            where
            u    = reduceEtas d span $ bindVarByName nam One (resolveMatches d span nam UNIM 0 "" [] (f (Var nam d)))
        BITM -> BitM fl tr 
            where
            fl   = reduceEtas d span $ bindVarByName nam Bt0 (resolveMatches d span nam BITM 0 "" [] (f (Var nam d)))
            tr   = reduceEtas d span $ bindVarByName nam Bt1 (resolveMatches d span nam BITM 1 "" [] (f (Var nam d)))
        NATM nams@[p] -> NatM z s 
            where
            z    = reduceEtas d span $ bindVarByName nam Zer (resolveMatches d span nam (NATM [p]) 0 "" [] (f (Var nam d)))
            s    = reduceEtas d span $ (Lam p (Just Nat) (\q -> 
                                       bindVarByName nam (Suc q) (resolveMatches (d+1) span nam (NATM nams) 1 "" [Sub q] (f (Var nam d)))))
        LSTM nams@[h,t] -> LstM nil cons 
            where
            nil  = reduceEtas d span $ bindVarByName nam Nil (resolveMatches d span nam (LSTM nams) 0 "" [] (f (Var nam d)))
        ...
        ENUM syms compl nams@[de] -> EnuM cases def 
          where
          cases = map (\sym -> (sym, reduceEtas d span $ bindVarByName nam (Sym sym) (resolveMatches d span nam (ENUM syms compl nams) 0 sym [] (f (Var nam d))))) syms
          def = if compl
                then reduceEtas d span (Lam de Nothing (\q -> bindVarByName nam q (resolveMatches (d+1) span nam (ENUM syms compl nams) (-1) "" [Sub q] (f (Var nam d)))))
                else One
...
resolveMatches d span nam elim clause sym args (Loc span' term) = resolveMatches d span' nam elim clause sym args term
resolveMatches d span nam elim clause sym args t = case t of
  App f x      ->
    case cut x of
      (Var k i) -> if k == nam
        then case (cut f, elim, args) of
          (UniM u       , UNIM      , [])    -> resolveMatches d span nam elim clause sym args $ u
          (BitM fl tr   , BITM      , [])    -> resolveMatches d span nam elim clause sym args $ ([fl, tr] !! clause)
          (NatM z s     , NATM _    , [])    -> resolveMatches d span nam elim clause sym args $ z
          (NatM z s     , NATM _    , [p])   -> resolveMatches d span nam elim clause sym args $ appInnerArg 0 0 (getSpan span s) s p
          (EmpM         , EMPM      , [])    -> resolveMatches d span nam elim clause sym args EmpM
          (LstM nil cons, LSTM _    , [])    -> resolveMatches d span nam elim clause sym args $ nil
          (LstM nil cons, LSTM _    , [h,t]) -> resolveMatches d span nam elim clause sym args $ appInnerArg 0 0 (getSpan span cons)  
                                                                                                 (appInnerArg 1 0 (getSpan span cons) cons t) h
          (SigM pair    , SIGM _    , [a,b]) -> resolveMatches d span nam elim clause sym args $ appInnerArg 0 0 (getSpan span pair)
                                                                                                 (appInnerArg 1 0 (getSpan span pair) pair b) a
          (EqlM refl    , EQLM      , [])    -> resolveMatches d span nam elim clause sym args refl
          (EnuM cs def  , ENUM _ _ _, [q])   -> resolveMatches d span nam elim clause sym args $ appInnerArg 0 0 (getSpan span def) def q
          (EnuM cs def  , ENUM _ _ _, [])    -> case lookup sym cs of
                                Just branch  -> resolveMatches d span nam elim clause sym args $ branch
                                Nothing      -> resolveMatches d span nam elim clause sym args $ appInnerArg 0 0 (getSpan span def) def (Var k i)
          _ -> App (resolveMatches d span nam elim clause sym args f) (resolveMatches d span nam elim clause sym args x)
        else   App (resolveMatches d span nam elim clause sym args f) (resolveMatches d span nam elim clause sym args x)
      _     -> App (resolveMatches d span nam elim clause sym args f) (resolveMatches d span nam elim clause sym args x)
  -- Not an eliminator application, just propagate deeper
  Lam k ty f   -> Lam k (fmap (resolveMatches d span nam elim clause sym args) ty) (\x -> resolveMatches (d+1) span nam elim clause sym args (f x))
  Var k i      -> Var k i
  Ref k i      -> Ref k i
  Sub t'       -> Sub (resolveMatches d span nam elim clause sym args t')
  Fix k f      -> Fix k (\x -> resolveMatches (d+1) span nam elim clause sym args (f x))
  Let k ty v f -> Let k (fmap (resolveMatches d span nam elim clause sym args) ty) (resolveMatches d span nam elim clause sym args v) (\x -> resolveMatches (d+1) span nam elim clause sym args (f x))
  Use k v f    -> Use k (resolveMatches d span nam elim clause sym args v) (\x -> resolveMatches (d+1) span nam elim clause sym args (f x))
  Set          -> Set
  Chk t' ty    -> Chk (resolveMatches d span nam elim clause sym args t') (resolveMatches d span nam elim clause sym args ty)
  Emp          -> Emp
  EmpM         -> EmpM
  Uni          -> Uni
  One          -> One
  UniM f       -> UniM (resolveMatches d span nam elim clause sym args f)
  Bit          -> Bit
  Bt0          -> Bt0
  Bt1          -> Bt1
  BitM f g     -> BitM (resolveMatches d span nam elim clause sym args f) (resolveMatches d span nam elim clause sym args g)
  Nat          -> Nat
  Zer          -> Zer
  Suc t'       -> Suc (resolveMatches d span nam elim clause sym args t')
  NatM z s     -> NatM (resolveMatches d span nam elim clause sym args z) (resolveMatches d span nam elim clause sym args s)
  Lst ty       -> Lst (resolveMatches d span nam elim clause sym args ty)
  Nil          -> Nil
  Con h t'     -> Con (resolveMatches d span nam elim clause sym args h) (resolveMatches d span nam elim clause sym args t')
  LstM nil c   -> LstM (resolveMatches d span nam elim clause sym args nil) (resolveMatches d span nam elim clause sym args c)
  Enu syms     -> Enu syms
  Sym s        -> Sym s
  EnuM cs def  -> EnuM (map (\(s,t') -> (s, resolveMatches d span nam elim clause sym args t')) cs) (resolveMatches d span nam elim clause sym args def)
  Num typ      -> Num typ
  Val v        -> Val v
  Op2 op a b   -> Op2 op (resolveMatches d span nam elim clause sym args a) (resolveMatches d span nam elim clause sym args b)
  Op1 op a     -> Op1 op (resolveMatches d span nam elim clause sym args a)
  Sig a b      -> Sig (resolveMatches d span nam elim clause sym args a) (resolveMatches d span nam elim clause sym args b)
  Tup a b      -> Tup (resolveMatches d span nam elim clause sym args a) (resolveMatches d span nam elim clause sym args b)
  SigM f       -> SigM (resolveMatches d span nam elim clause sym args f)
  All a b      -> All (resolveMatches d span nam elim clause sym args a) (resolveMatches d span nam elim clause sym args b)
  Eql ty a b   -> Eql (resolveMatches d span nam elim clause sym args ty) (resolveMatches d span nam elim clause sym args a) (resolveMatches d span nam elim clause sym args b)
  Rfl          -> Rfl
  EqlM f       -> EqlM (resolveMatches d span nam elim clause sym args f)
  Rwt e f      -> Rwt (resolveMatches d span nam elim clause sym args e) (resolveMatches d span nam elim clause sym args f)
  Met name ty as -> Met name (resolveMatches d span nam elim clause sym args ty) (map (resolveMatches d span nam elim clause sym args) as)
  Era          -> Era
  Sup l' a b   -> Sup (resolveMatches d span nam elim clause sym args l') (resolveMatches d span nam elim clause sym args a) (resolveMatches d span nam elim clause sym args b)
  SupM l' f    -> SupM (resolveMatches d span nam elim clause sym args l') (resolveMatches d span nam elim clause sym args f)
--Loc span t'  -> Loc span (resolveMatches d span nam l clause sym args t')
  Log s x      -> Log (resolveMatches d span nam elim clause sym args s) (resolveMatches d span nam elim clause sym args x)
  Pri pf       -> Pri pf
  Pat ss ms cs -> Pat (map (resolveMatches d span nam elim clause sym args) ss)
                      (map (\(m,t') -> (m, resolveMatches d span nam elim clause sym args t')) ms)
                      (map (\(ps,rhs) -> (map (resolveMatches d span nam elim clause sym args) ps, resolveMatches d span nam elim clause sym args rhs)) cs)
  Frk l' a b   -> Frk (resolveMatches d span nam elim clause sym args l') 
                      (resolveMatches d span nam elim clause sym args a) 
                      (resolveMatches d span nam elim clause sym args b)
  Tst x        -> Tst (resolveMatches d span nam elim clause sym args x)
    ...
    case cut x of
      (Var k _) -> if k == nam
       ...
       -- SupM l _    -> not eliminated
          ...
          NatM _ s    -> NATM (getVarNames 1 span s) <+> isEtaLong d span nam f
          EmpM        -> EMPM <+> isEtaLong d span nam f
          LstM _ c    -> LSTM (getVarNames 2 (getSpan span c) c) <+> isEtaLong d span nam f
          SigM g      -> SIGM (getVarNames 2 span g) <+> isEtaLong d span nam f
...
data ElimLabel
  ...
  | EQLM
  | ENUM [String] Bool [String] -- [symbolNames], isComplete, [defaultArgName]
  | NONE
...
-- 2) Parameter `j` counts leading lambdas before the eliminator (unused here, always 0)
-- 3) When i=0, j=0: applies the current term to `arg'
-- 4) When i>0: descends into the eliminator's branches, adjusting for their arities
--    4.1) NatM's successor branch expects 1 extra arg, so uses (l-1+1)
--    4.2) LstM's cons branch expects 2 extra args, so uses (l-1+2)
-- 5) App nodes increment `i` to track depth in the application spine
--

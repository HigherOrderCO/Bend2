...
-- an argument of a constructor in a match by a var of the newly-pulled-outward match of the reduction
  ...
  Lam k mty f ->
    case isEtaLong d k (f (Var k d)) of
        Just lab -> case lab of
          ...
          ENUM syms -> EnuM cases Nothing where  -- No default case in eta-reduced form
  ...
  Enu ss      -> Enu ss
  Sym s       -> Sym s
  EnuM cs def -> EnuM [(s, reduceEtas d v) | (s,v) <- cs] (fmap (reduceEtas d) def)
  Num nt    -> Num nt
  Val nv    -> Val nv
...
resolveMatches d n l clause args t = case t of
  ...
  Tst t' -> Tst (resolveMatches d n l clause args t')
  Emp -> Emp
  EmpM -> EmpM
  Uni -> Uni
  One -> One
  ...
  EnuM cs def -> EnuM (map (\(s,t') -> (s, resolveMatches d n l clause args t')) cs) 
                      (fmap (resolveMatches d n l clause args) def)
  Num typ -> Num typ
  Val v -> Val v
        ...
        then case (cut f, l) of
          ...
          (EnuM cs def, ENUM syms) ->
            if clause == -1 
              ...
              then case def of
                Nothing -> Era  -- No default case available, return Era (unreachable)
                Just defTerm -> case (cut defTerm, args) of
                 (Lam _ _ body, [q]) -> resolveMatches d n l clause args (body q)
                 _ -> foldl App (resolveMatches d n l clause args defTerm) args
              else 
...
isEtaLong d n t = case t of
  ...
  Enu _ -> Nothing
  Sym _ -> Nothing
  EnuM cs def -> foldr (<|>) (maybe Nothing (isEtaLong d n) def) (map (isEtaLong d n . snd) cs)
  Num _ -> Nothing
  Val _ -> Nothing

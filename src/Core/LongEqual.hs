...
longEqual :: Int -> Book -> Term -> Term -> Bool
longEqual d book a b =
  let res = eql True d book a b
  in 
    -- trace ("- longEqual: " ++ show (normal book a) ++ " == " ++ show (normal book b) ++ " -> " ++ show res) $
    res
...
cmp red d book a b =
  let res = case (cut a , cut b) of
            ...
            --   _                    -> False
            (EnuM ca da, EnuM cb db) -> 
              let 
                allLabels = S.toList $ S.union (S.fromList $ map fst ca) (S.fromList $ map fst cb)
                checkLabel label = 
                  case (lookup label ca, lookup label cb) of
                    (Just bodyA, Just bodyB) -> eql red d book bodyA bodyB
                    (Just bodyA, Nothing) -> eql red d book (Lam "_" Nothing (\_ -> bodyA)) db
                    (Nothing, Just bodyB) -> eql red d book da (Lam "_" Nothing (\_ -> bodyB))
                    (Nothing, Nothing) -> True
              in all checkLabel allLabels && eql red d book da db
            ...
            -- (App (cut -> EnuM cs def) (cut -> xa), (cut -> b)) | not (isEnuMApp b) ->
            --   let replaceInnermost term = case cut term of
            --         Sig ty (Lam k mt body) -> Sig ty (Lam k mt (\x -> replaceInnermost (body x)))
            --         _ -> b
            --       newClauses = map (\(label, body) -> (label, replaceInnermost body)) cs
            --       newDefault = Lam "_" Nothing (\_ -> b)
            --   in eql red d book a (App (EnuM newClauses newDefault) xa)
            --
            ...
            --         _ -> b
            --       newClauses = map (\(label, body) -> (label, replaceInnermost body)) cs
            --       -- Check if EnuM is complete by looking at the argument type
            --       isComplete = case cut xa of
            --         Sym s -> case findEnumTypeForSymbol book s of
            --           Just (Enu syms) -> 
            --             let covered = map fst cs
            --             in length covered >= length syms && all (`elem` syms) covered
            --           _ -> False
            --         _ -> False
            --       newDefault = if isComplete 
            --                    then def  -- Keep original default (likely λ_. ())
            --                    else Lam "_" Nothing (\_ -> b)
            --   in eql red d book a (App (EnuM newClauses newDefault) xa)
            (App (cut -> EnuM cs def) (cut -> xa), (cut -> b)) | not (isEnuMApp b) ->
              let 
                  isComplete = if not (length cs > 0)
                               then False
                               else case findEnumTypeForSymbol book (fst (head cs)) of
                                  Just (Enu syms) -> 
                                    let covered = map fst cs
                                    in length covered == length syms && all (`elem` syms) covered
                                  _ -> False
                  newClauses = map (\(label, body) -> (label, b)) cs
                  newDefault = if isComplete 
                               then def -- Keep original default (likely λ_. ())
                               else Lam "_" Nothing (\_ -> b)
              in eql red d book a (App (EnuM newClauses newDefault) xa)
...
findEnumTypeForSymbol :: Book -> String -> Maybe Term
findEnumTypeForSymbol (Book defs _) sym = 
  listToMaybe [ Enu tags | (_, (_, Sig (Enu tags) _, _)) <- M.toList defs, sym `elem` tags ]
...
isEnuMApp :: Term -> Bool
isEnuMApp (cut -> App (cut -> EnuM _ _) _) = True
isEnuMApp _ = False

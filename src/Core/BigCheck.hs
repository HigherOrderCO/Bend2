...
-- COMPLETE MINIMAL UNIFICATION IMPLEMENTATION
-- Replace parts of your current code with this

-- -- 1. Keep your Constraint type but enhance it slightly
-- data Constraint
--   = MustEqual Name Int Term           -- Variable must equal this type
--   | MustAcceptArg Name Int Term       -- Variable must be function accepting this
--   deriving Show
--
-- -- 2. Add new type for substitutions
-- type Subst = [(Int, Term)]
--
-- -- 3. Add function to apply substitutions (minimal version)
-- applySubst :: Subst -> Term -> Term
-- applySubst [] t = t
-- applySubst subst (Var n i) | i < 0 =  -- Negative indices are type variables
--   case lookup i subst of
--     Just t -> applySubst subst t
--     Nothing -> Var n i
-- applySubst subst (All a b) = All (applySubst subst a) (applySubst subst b)
-- applySubst subst (App f x) = App (applySubst subst f) (applySubst subst x)
-- applySubst subst (Num nt) = Num nt
-- applySubst subst Nat = Nat
-- applySubst subst Bit = Bit
-- applySubst _ t = t  -- For now, don't go deeper into other constructors
--
-- -- 4. Convert constraint to equation
-- -- Use the actual depth and variable name to make it unique
-- constraintToEquation :: Constraint -> (Term, Term)
-- constraintToEquation (MustEqual n i t) = 
--   (Var n i, t)
-- constraintToEquation (MustAcceptArg n i argT) = 
--   -- Use the variable's own index to make the return type unique
--   let retVar = Var ("?ret_" ++ n ++ "_at_" ++ show i) (i - 1)
--   in (Var n i, All argT retVar)
--
-- -- 5. Basic unification function
-- unify :: Term -> Term -> Maybe Subst
-- unify t1 t2 = case (cut t1, cut t2) of
--   -- Type variable unification
--   (Var n i, t) | i < 0 -> Just [(i, t)]
--   (t, Var n i) | i < 0 -> Just [(i, t)]
--
--   -- Same types unify with empty substitution
--   (Nat, Nat) -> Just []
--   (Bit, Bit) -> Just []
--   (Num n1, Num n2) | n1 == n2 -> Just []
--
--   -- Function types - unify parts
...
--
--   -- Otherwise fail
--   _ -> trace ("failing to unify " ++ show t1 ++ " and " ++ show t2) $  Nothing
--
-- -- 6. Unify a list of equations
-- unifyAll :: [(Term, Term)] -> Maybe Subst
-- unifyAll [] = Just []
-- unifyAll ((t1, t2):rest) =  trace ("DDD") $ do
--   s1 <- trace "HHH " $ unify t1 t2
--   let rest' = trace "GGG" $ [(applySubst s1 a, applySubst s1 b) | (a, b) <- rest]
--   s2 <- trace "FFF" $ unifyAll rest'
--   return $ trace "EEEE" $ (s1 ++ s2)
--
-- -- 7. Replace your solveConstraints with this
-- -- Better: use the actual constraint index
-- solveConstraintsUnify :: [Constraint] -> Maybe Subst
-- solveConstraintsUnify constraints =
--   let equations = zipWith constraintToEquation [0..] constraints
--   in unifyAll equations
--
-- -- 8. Replace collectSimpleConstraints with this
-- collectConstraintsRich :: Int -> Book -> Ctx -> Name -> Int -> Term -> [Constraint]
-- collectConstraintsRich d book ctx targetName targetDepth term = 
--   case cut term of
--     App f (cut -> Var k i) | k == targetName && i == targetDepth ->
--       -- Variable used as argument
--       case force book <$> infer d noSpan book ctx f Nothing of
--         Done (All argType _) -> [MustEqual k i argType]
--         _ -> []
--
--     App (cut -> Var k i) x | k == targetName && i == targetDepth ->
--       -- Variable used as function
--       case infer d noSpan book ctx x Nothing of
--         Done xType -> [MustAcceptArg k i xType]
--         _ -> []
--
--     App f x -> 
--       -- Recurse into both parts
--       collectConstraintsRich d book ctx targetName targetDepth f ++
--       collectConstraintsRich d book ctx targetName targetDepth x
--
--     Lam k mt body ->
--       -- Go into lambda body
--       let newCtx = extend ctx k (Var k d) (fromMaybe (Var ("?" ++ k) (-d-100)) mt)
--       in collectConstraintsRich (d+1) book newCtx targetName targetDepth (body (Var k d))
--
--     _ -> []
--
-- -- Make type variables explicit with "?" prefix
-- makeTypeVar :: Name -> Int -> Term
-- makeTypeVar name depth = Var ("?" ++ name ++ "_" ++ show depth) (-depth-1)
--
-- isTypeVar :: Term -> Bool
-- isTypeVar (Var n _) = case n of
--   ('?':_) -> True
--   _ -> False
-- isTypeVar _ = False
--
-- collectConstraintsForParam :: Int -> Book -> Ctx -> Name -> Int -> Int -> Term -> [Constraint]
-- collectConstraintsForParam d book ctx targetName termIdx typeIdx term = 
--   case cut term of
--     App f (cut -> Var k i) | k == targetName && i == termIdx ->
--       case inferStructural d noSpan book ctx f of
--         Done (All argType _) -> 
--           if containsTypeVars argType
--           then []  -- Skip constraints with other type variables
--           else [MustEqual targetName typeIdx argType]
--         _ -> []
--
--     App (cut -> Var k i) x | k == targetName && i == termIdx ->
--       case inferStructural d noSpan book ctx x of
--         Done xType -> 
--           if containsTypeVars xType
--           then []
--           else [MustAcceptArg targetName typeIdx xType]
--         _ -> []
--
--     App f x -> 
--       collectConstraintsForParam d book ctx targetName termIdx typeIdx f ++
--       collectConstraintsForParam d book ctx targetName termIdx typeIdx x
--
--     Lam k mt body ->
--       -- For unknowns, use type variable with "?" prefix
--       let tyForCtx = case mt of
--             Just t -> t
--             Nothing -> makeTypeVar k d
--       in collectConstraintsForParam (d+1) book (extend ctx k (Var k d) tyForCtx)
--                                    targetName termIdx typeIdx (body (Var k d))
--
--     _ -> []
--
-- -- Check if contains type variables (anything starting with "?")
-- containsTypeVars :: Term -> Bool
-- containsTypeVars (Var n _) = case n of
--   ('?':_) -> True
--   _ -> False
-- containsTypeVars (All a b) = containsTypeVars a || containsTypeVars b
-- containsTypeVars (App f x) = containsTypeVars f || containsTypeVars x
-- containsTypeVars _ = False
--
-- -- Updated inferLambdaWithConstraints
-- inferLambdaWithConstraints :: Int -> Span -> Book -> Ctx -> Name -> Body -> Result Term
-- inferLambdaWithConstraints d span book ctx k body = do
--   let kTypeVar = makeTypeVar k d  -- Clean type variable
--   let bodyTerm = body (Var k d)
--   let ctxWithTypeVar = extend ctx k (Var k d) kTypeVar
--
--   -- Collect constraints - the type variable has index (-d-1)
--   let constraints = collectConstraintsForParam d book ctxWithTypeVar k d (-d-1) bodyTerm
--
--   traceM $ "constraints on " ++ k ++ " @ " ++ show bodyTerm ++ " -> " ++ show constraints
--   case solveConstraintsUnify constraints of
--     Just subst ->
--       let paramType = applySubst subst kTypeVar
--       in if isTypeVar paramType
--          then Fail $ CantInfer span (normalCtx book ctx) Nothing
--          else do
--            bodyType <- infer (d+1) span book 
--                             (extend ctx k (Var k d) paramType)
--                             bodyTerm
--                             Nothing
--            return $ All paramType (Lam "_" (Just paramType) (\_ -> bodyType))
--     Nothing ->
...
-- -- collectConstraintsForParam d book ctxWithTypeVar k d (-d-1) bodyTerm
-- collectConstraintsForParam :: Int -> Book -> Ctx -> Name -> Int -> Int -> Term -> [Constraint]
-- collectConstraintsForParam d book ctx targetName termIdx typeIdx term = 
--   case cut term of
--     App f (cut -> Var k i) | k == targetName && i == termIdx ->
...
-- inferLambdaWithConstraints :: Int -> Span -> Book -> Ctx -> Name -> Int -> Body -> Result Term
-- inferLambdaWithConstraints d span book ctx k i body = do
--   traceM $ "infer lambda cstr: " ++ show (Var k i) ++ " @ " ++ show (body (Var k i))
--   let ctxWithTypeVar = extend ctx k (Var k i) (Var ("?"++k) (-i-1))
--   let constraints = collectConstraintsForParam d book ctxWithTypeVar k d (-d-1) bodyTerm
--
--   Fail $ CantInfer span (normalCtx book ctx) Nothing
...
infer d span book ctx term environment = 
  ...
  let (_, substs) = getSubsts d book constraints []
  ...
  --   Done t -> 
  --     -- trace ("inferred structural " ++ show term ++ " @ " ++ show t) $ 
  --     Done t
  --   Fail _ -> 
  --     -- Structural failed, try constraint-based for specific patterns
  --     case cut term of
  --       Lam k Nothing body -> do
  --         (typeWithNewVars, constraints) <- inferWithConstraints d span book ctx (Lam k Nothing body)
  --         let (_, substs) = getSubsts d book constraints []
  --         let termType = doSubsts substs typeWithNewVars
  --         -- traceM $ "infered with constraints that " ++ show term ++ " :: " ++ show termType
  --         return termType
  --
  --       -- -- For other cases, use environment if available
  --       -- Var name idx -> case environment of
  --       --   Just env -> inferFromUsage d span book ctx name idx env
...
getSubsts d book constraints substitutions = error $ "undefined "  ++ show constraints ++ " -- " ++ show substitutions
...
-- -- 10. Keep your existing infer function EXACTLY as is
-- infer :: Int -> Span -> Book -> Ctx -> Term -> Maybe Term -> Result Term
-- infer d span book ctx term environment = 
--   case inferStructural d span book ctx term of
--     Done t -> Done t
...
--       case cut term of
--         Lam k Nothing body -> 
--           -- inferLambdaWithConstraints d span book ctx k body
--           inferLambdaWithConstraints d span book ctx k d body
--
--         -- For other cases, use environment if available
--         Var name idx -> case environment of
--           Just env -> inferFromUsage d span book ctx name idx env
...
--         _ -> Fail $ CantInfer span (normalCtx book ctx) Nothing
--
-- -- 11. Keep inferFromUsage EXACTLY as you have it
-- inferFromUsage :: Int -> Span -> Book -> Ctx -> Name -> Int -> Term -> Result Term
-- inferFromUsage d span book ctx name idx environment =
--   case environment of
--     App f x | isTarget x ->
--       -- Variable is argument to f
--       case inferStructural d span book ctx f of
--         Done (All argType _) -> Done argType
--         _ -> Fail $ CantInfer span (normalCtx book ctx) Nothing
--
--     _ -> Fail $ CantInfer span (normalCtx book ctx) Nothing
--   where
--     isTarget (Var n i) = n == name && i == idx
--     isTarget _ = False

-- DELETE these from your code:
-- - solveSimple
-- - collectSimpleConstraints
-- - the old solveConstraints

-- KEEP these from your code:
-- - collectUnknowns (though we're not using it yet)
-- - collectConstraints (though we're not using it yet)
-- - inferWithConstraints (though we're not using it yet)
...
--     Ref k i -> do
--       case getDefn book k of
--         Just (_, _, typ) -> trace ("infered " ++ k ++ " :: " ++ show typ) $ return typ
--         Nothing          -> trace ("failed " ++ k ++ " :: ")Fail $ Undefined span (normalCtx book ctx) k Nothing
--
--     -- ctx |- x : T
--     -- ------------ infer-Sub
--     -- ctx |- x : T
...
--           fT <- 
--             -- trace ("AAAA " ++ show f ++ " :: ? ") $ 
--             inferStructural d span book ctx f
--           case 
--             -- trace ("in " ++ show term ++ " , f :: " ++ show fT) $
--             force book fT of
--             All fA fB -> 
--               -- trace ("HERE " ++ show term) $ 
--               do
--               _ <- 
--                 -- trace ("must check: " ++ show x ++ " :: " ++ show fA) $ 
--                 check d span book ctx x fA
--               return $ 
--                 trace ("infered structural app that " ++ show term ++ " :: " ++ show (App fB x)) $
--                 (App fB x)
--             _ -> 
--               -- trace ("THERE " ++ show term) $ 
--               do

...
check d span book ctx term      goal =
  trace ("-check: " ++ show term ++ " :: " ++ show (force book goal) ++ "\n-ctx:\n" ++ show ctx) $
  case (term, force book goal) of
    ...
    (Lam k t f, All a b) -> do
      fb' <- check (d+1) span book (extend ctx k (Var k d) a) (f (Var k d)) (App b (Var k d))
      return $ Lam k t (\v -> bindVarByIndex d v fb')
    ...
    (NatM z s, All (force book -> Nat) rT) -> trace "AAAAAAAA" $ do
    ...
    -- (App (Lam a mt b) x, _) -> do
    --   xT <- infer d span book ctx x
    --   check d span book ctx ()
    --   check d span book ctx f (All xT (Lam "_" Nothing (\_ -> All xT (Lam "_" Nothing (\_ -> goal)))))
    
    (App fn@(NatM z s) x, _) -> do
      x' <- check d span book ctx x Nat
      traceM $ "NATM: " ++ show goal
      traceM $ "NATM: " ++ show (rewrite d book x Zer goal)
      z' <- check d span book (rewriteCtx d book x Zer ctx) z (rewrite d book x Zer goal)
      case cut s of
        Lam p mt b -> do
          let pV = Var p d
          sb' <- check (d+1) span book (extend ctx p pV Nat) (b pV) (rewrite d book x (Suc pV) goal)
          return $ App (NatM z' (Lam p mt (\v -> bindVarByIndex d v sb'))) x'
        _ -> Fail $ TermMismatch (getSpan span s) (normalCtx book ctx) (normal book (Lam "_" Nothing (\_ -> Var "_" 0))) (normal book (Var "_" 0)) Nothing

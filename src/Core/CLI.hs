...
-- Type-check all definitions in a book
checkBook :: Book -> IO Book
checkBook book@(Book funs adts names) = do
  let orderedFuns = [(name, fromJust (M.lookup name funs)) | name <- names, M.member name funs]
  success <- checkAll book orderedFuns
  unless success exitFailure
  return book
  where
    checkDef bk term typ = do
      ...
      check 0 noSpan bk (Ctx []) term typ
      return ()
    checkAll :: Book -> [(Name, Function)] -> IO Bool
    checkAll bk [] = return True
    checkAll bk ((name, Function _ typ term):rest) = do
      case checkDef bk term typ of
        Done () -> do
          putStrLn $ "\x1b[32m✓ " ++ name ++ "\x1b[0m"
          checkAll bk rest
...
getGenDeps file = do
  book <- parseFile file
  let bookAdj@(Book funs adts names) = adjustBook book

  -- Find all definitions that are `try` definitions (i.e., contain a Met)
  let tryFuns = M.filter (\(Function _ _ term) -> hasMet term) funs
  let tryNames = M.keysSet tryFuns

  -- Find all reverse dependencies (examples)
  let allFuns = M.toList funs
  let reverseDeps = S.fromList [ name | (name, Function _ typ term) <- allFuns, not (name `S.member` tryNames), not (S.null (S.intersection tryNames (S.union (getDeps term) (getDeps typ)))) ]

  -- Get all dependencies of the `try` definitions and the reverse dependencies
  let targetFuns = M.filterWithKey (\k _ -> k `S.member` tryNames || k `S.member` reverseDeps) funs
  let allDeps = S.unions $ map (\(Function _ typ term) -> S.union (getDeps term) (getDeps typ)) (M.elems targetFuns)
  ...
  -- Filter the book to get the definitions we want to print
  let finalFuns = M.filterWithKey (\k _ -> k `S.member` finalDepNames) funs
  let finalNames = filter (`S.member` finalDepNames) names
  
  -- Print the resulting book
  print $ Book finalFuns adts finalNames

-- | Collect all refs from a Book
collectAllRefs :: Book -> S.Set Name
collectAllRefs (Book funs _ _) = 
  S.unions $ map collectRefsFromFun (M.elems funs)
  where
    collectRefsFromFun (Function _ typ term) = S.union (getDeps term) (getDeps typ)
...
hasMet term = case term of
  ...
  Con h t     -> hasMet h || hasMet t
  LstM n c    -> hasMet n || hasMet c
  ADT _ args  -> any hasMet args
  Ctr _ args  -> any hasMet args
  ADTM _ cs e -> any (hasMet . snd) cs || maybe False hasMet e
  Op2 _ a b   -> hasMet a || hasMet b
  Op1 _ a     -> hasMet a
  Sig a b     -> hasMet a || hasMet b
  Tup a b     -> hasMet a || hasMet b

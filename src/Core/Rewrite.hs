...
-- TODO: refactor the Core.Rewrite module
-- module Core.Rewrite where
...
-- rewriteGo d book old neo val =
  -- case val of
    ...
    -- ADTM t cs d -> ADTM t (map (\(n,c) -> (n, rewrite d book old neo c)) cs) (fmap (rewrite d book old neo) d)
    -- Num t       -> Num t
    -- Val v       -> Val v
    -- Op2 o a b   -> Op2 o (rewrite d book old neo a) (rewrite d book old neo b)
    -- Op1 o a     -> Op1 o (rewrite d book old neo a)
...
-- src/Core/Rewrite.hs:138:55: error: [GHC-83865]
    -- • Couldn't match expected type ‘Int’ with actual type ‘Maybe Term’
    -- • In the first argument of ‘rewrite’, namely ‘d’
      -- In the expression: rewrite d book old neo c
      -- In the expression: (n, rewrite d book old neo c)
    -- |
-- 138 |     ADTM t cs d -> ADTM t (map (\(n,c) -> (n, rewrite d book old neo c)) cs) (fmap (rewrite d book old neo) d)
    -- |                                                       ^

-- src/Core/Rewrite.hs:138:93: error: [GHC-83865]
    -- • Couldn't match expected type ‘Int’ with actual type ‘Maybe Term’
    -- • In the first argument of ‘rewrite’, namely ‘d’
      -- In the first argument of ‘fmap’, namely ‘(rewrite d book old neo)’
      -- In the third argument of ‘ADTM’, namely
        -- ‘(fmap (rewrite d book old neo) d)’
    -- |
-- 138 |     ADTM t cs d -> ADTM t (map (\(n,c) -> (n, rewrite d book old neo c)) cs) (fmap (rewrite d book old neo) d)
    -- |                                                                                             ^

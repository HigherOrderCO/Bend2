...
whnfRef book k i =
  if i == 0
  then Ref k i
  else case getFun book k of
    Just fn -> deref book k i (LHS SZ id) (funBody fn)
    Nothing -> Ref k 0
...
whnfApp book f x =
  case whnf book f of
    ...
    NatM z s   -> whnfNatM book x z s
    LstM n c   -> whnfLstM book x n c
    ADTM n c d -> whnfADTM book x n c d
    SigM f     -> whnfSigM book x f
    SupM l f   -> whnfSupM book x l f
    EqlM f     -> whnfEqlM book x f
    Frk _ _ _  -> error "unreachable"
...
-- Normalizes an ADT match
whnfADTM :: Book -> Term -> Name -> [(Name,Term)] -> Maybe Term -> Term
whnfADTM book x n c d =
  case whnf book x of
    Ctr k args -> case lookup k c of
      Just t  -> whnf book (foldl App t args)
      Nothing -> case d of
        Just df -> whnf book (App df (Ctr k args))
        Nothing -> App (ADTM n c d) (Ctr k args)
    x' -> App (ADTM n c d) x'
...
deref book k i lhs body =
  case body of
    ...
    ADTM n c d ->
      Lam "x" Nothing $ \x -> case whnf book x of
        Ctr k' args -> case lookup k' c of
          Just t  -> deref book k i lhs (foldl App t args)
          Nothing -> case d of
            Just df -> deref book k i lhs (App df (Ctr k' args))
            Nothing -> derefUndo book k i lhs (ADTM n c d) x
        x' -> derefUndo book k i lhs (ADTM n c d) x'
    Lam n t f ->
      Lam n t $ \x -> deref book k i (lhs_ctr lhs SZ x) (f x)
    t -> t
...
normal book term =
  ...
  case whnf book term of
    ...
    Con h t     -> Con (normal book h) (normal book t)
    LstM n c    -> LstM (normal book n) (normal book c)
    ADT n ps    -> ADT n (map (normal book) ps)
    Ctr k args  -> Ctr k (map (normal book) args)
    ADTM n c d  -> ADTM n (map (\(s, t) -> (s, normal book t)) c) (fmap (normal book) d)
    Sig a b     -> Sig (normal book a) (normal book b)
    Tup a b     -> Tup (normal book a) (normal book b)
    SigM f      -> SigM (normal book f)
    All a b     -> All (normal book a) (normal book b)

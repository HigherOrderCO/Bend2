...
whnfRef book k i =
  ...
  then Ref k i
    ...
    -- Just (False, term, _) -> trace ("DEREFING " ++ k ++ " TO " ++ show (whnf book $ deref book k i (LHS SZ id) term)) $ whnf book $ deref book k i (LHS SZ id) term
    otherwise             -> Ref k 0
...
deref2 :: Book -> String -> Int -> LHS -> Term -> Term
deref2 book ref i lhs body =
  case body of
    EmpM ->
      Lam "x" Nothing $ \x -> App (go body) x
    UniM f ->
      Lam "x" Nothing $ \x -> App (go body) x
    BitM f t ->
      Lam "x" Nothing $ \x -> App (go body) x
  ...
  go term = case term of
    Var k i     -> Var k i
    Ref k i     -> if k == ref then Ref k 0 else Ref k i
    Sub t       -> t
